...
- **Materiale:** disponibile su [GitHub](https://andrerecio.github.io/econometrics-undergraduate/) e Moodle
...
Sfruttare le esercitazioni per imparare a usare R, manipolare i data, costruire grafici e interpretare i risultati
...
Ogni tutorial è composta da:

- File **HTML** — materiale di riferimento con codice e spiegazioni
- **Script `.R`** — codice che potete eseguire (**run!**) se volete esercitarvi


Su [GitHub](https://andrerecio.github.io/econometrics-undergraduate/) trovate entrambi i file: HTML e script R.
...
`tidyverse` è una raccolta di pacchetti R per analisi dei data:

- `dplyr` — manipolare i data
- `ggplot2` — grafici
- `tidyr` — riorganizzare i data
...
## 🧹 Tidying — manipolare i data: `dplyr`



1. `filter()`: seleziona observations in base a una condizione
...
5. `summarise()`: riduce i data a un unico riepilogo
...
## 🧹 Tidying — manipolare i data

Creiamo un nuovo dataset: solo chi ha più di 12 years of education:
...
- `|>` o `%>%` è l'operatore **pipe**: passa l'output di un'espressione come input della function successiva, concatenando più operazioni
...
- `n()` conta le observations per gruppo
...
- [Codebook](https://github.com/andrerecio/econometrics-undergraduate/blob/main/introduction/codebook_crime2.md) con le variabili
...
- La figure mostra una correlazione **positiva** tra polizia e crimine: più polizia, più crimine?
...
- La **covarianza** misura l'associazione linear tra due variabili
